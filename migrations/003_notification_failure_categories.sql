-- Crowdshield schema version 3. Aligns durable notification failures with the closed ops vocabulary.

CREATE TABLE notification_delivery_state_v3 (
    ...
    failure_category TEXT NOT NULL DEFAULT '' CHECK (failure_category IN (
        '', 'config', 'credential', 'feed_download', 'feed_validation',
        'lapi_auth', 'lapi', 'database', 'notification', 'ownership',
        'timeout', 'cancelled', 'runtime', 'internal'
    )),
...
INSERT INTO notification_delivery_state_v3(
    kind, key_feed, failure_category, state_feed, consecutive_failures,
    notified, recovery_pending, sent, last_attempt_at, updated_at
)
SELECT kind, key_feed, failure_category, state_feed, consecutive_failures,
       notified, recovery_pending, sent, last_attempt_at, updated_at
FROM notification_delivery_state;

DROP TABLE notification_delivery_state;
ALTER TABLE notification_delivery_state_v3 RENAME TO notification_delivery_state;
